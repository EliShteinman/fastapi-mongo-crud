...
REM ================================================================================
REM  Deploys the FastAPI service using the standard DEPLOYMENT approach.
REM ================================================================================
...
echo.
echo ---^> Deploying MongoDB (Deployment)...
...
powershell -NoProfile -Command "(Get-Content -Raw %K8S_DIR%\05-fastapi-deployment.yaml) -replace 'docker.io/YOUR_DOCKERHUB_USERNAME/fastapi-mongo-crud:latest', '%FULL_IMAGE_NAME%' | oc apply -f -"
IF %ERRORLEVEL% NEQ 0 (
